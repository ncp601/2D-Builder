...
public class TabbedPane implements ChangeListener{
    ...
    private AbstractFloorComponentFactory kitchenFactory = FloorComponentFactoryProducer.getFactory("KITCHEN");
    private AbstractFloorComponentFactory dinningRoomFactory = FloorComponentFactoryProducer.getFactory("DINNINGROOM");
    private AbstractFloorComponentFactory livingRoomFactory = FloorComponentFactoryProducer.getFactory("LIVINGROOM");
		...
		if(m.find()){
			...
			while(i < loadComponents.length()){
				...
				if(loadComponents.charAt(i) == '|' && loadComponents.charAt(i+1) == '|'){
					...
					if(kitchenFactory.getGridComponent(loadedType.toString()) != null){
						loadedComponent = kitchenFactory.getGridComponent(loadedType.toString());
					}
					...
					if(livingRoomFactory.getGridComponent(loadedType.toString()) != null){
						loadedComponent = livingRoomFactory.getGridComponent(loadedType.toString());
					}
